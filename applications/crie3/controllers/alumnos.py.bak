...
def estadistica():
    ...
    contar_pdrs = 0
    contar_gpo = 0
    contar_grup={'a': 0,'b': 0,'c': 0,'d': 0,'e': 0,'f': 0}
    gpo=['a','b','c','d','e','f']
    gdo=['1','2','3','4','5','6']
    tab_alum=[]
    tab_gdo=[]
...
#despliega la estadistica por grado y grupo         
    despliegue = db((db.alumnos.aso_esp==asesor) & (db.alumnos.escuela==asesor2)).select(db.alumnos.ALL)
    for x in gdo:
        for Rows in despliegue:
            for g in gpo:
                if Rows.grupo == g and Rows.grado==x:
                    tab_alum.append(Rows.grupo)
                    tab_gdo.append(Rows.grado)
                    contar_grup[g]=contar_grup[g]+1
                    contar_gpo=contar_gpo + 1
    return dict(contador=contador, formulario=formulario, contar_esc = contar_esc,contar_pdrs=contar_pdrs,contar_gpo=contar_gpo,contar_doc = contar_doc, asesor=asesor,asesor2=asesor2,contar_grup=contar_grup,tab_alum=tab_alum,gpo=gpo,gdo=gdo,tab_gdo=tab_gdo,Rows=Rows)
